...
/**
 * Custom-styled Google sign-in/up control with a clickable GIS overlay.
 */
const GoogleSignInButton = ({
  ...
  useEffect(() => {
    ...
    const mount = () => {
      const buttonWidth = width || wrapper.clientWidth || 360;
      return googleSignIn.mountButton(overlay, { text, width: buttonWidth });
    };
  ...
  }, [disabled, googleSignIn, isSignup, width]);
  ...
  return (
    <div
      ref={wrapperRef}
      className={cn('relative w-full min-h-[44px]', className)}
      data-google-configured="true"
    >
      <div
        className={cn(
          'pointer-events-none flex w-full min-h-[44px] items-center justify-center gap-2 rounded-lg border border-border bg-background text-sm font-semibold text-foreground',
