...
const findExistingScopeId = async (tableName, tenantId, transaction = null) => {
  const rows = await sequelize.query(
    `SELECT id
     FROM ${quoteIdent(tableName)}
...
const processTenantScope = async ({ tenant, scope, targets }) => {
  ...
  const run = async (transaction = null) => {
    ...
    const scopeRow = isApply
      ...
      : { id: await findExistingScopeId(scopeTable, tenant.id, transaction) };
    ...
    return { action, scopeId: scopeRow?.id || null, tables };
  };
  ...
  console.log(
    `[${isApply ? 'apply' : 'dry-run'}] ${tenant.name} (${tenant.id}) ${scope} action=${result.action} ${scope}Id=${result.scopeId || 'missing'} rows=${total} tables=${formatTableCounts(result.tables)}`
  );
